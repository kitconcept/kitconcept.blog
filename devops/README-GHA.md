...
2. Go to your repository at [kitconcept/kitconcept-blog](https://github.com/kitconcept/kitconcept-blog).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `kitconcept-blog.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | kitconcept-blog.example.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | kitconcept-blog.example.com |
...
- [Backend Tests Workflow](https://github.com/kitconcept/kitconcept-blog/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/kitconcept/kitconcept-blog/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`kitconcept/kitconcept-blog-backend`) and Frontend (`kitconcept/kitconcept-blog-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of kitconcept-blog.example.com](https://github.com/kitconcept/kitconcept-blog/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/kitconcept-blog.example.com.yml` stack.
