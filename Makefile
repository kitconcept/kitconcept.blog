...
PROJECT_NAME=kitconcept-blog
STACK_NAME=kitconcept-blog-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://kitconcept-blog.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t kitconcept/kitconcept-blog-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t kitconcept/kitconcept-blog-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name kitconcept-blog-backend-acceptance -d kitconcept/kitconcept-blog-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name kitconcept-blog-frontend-acceptance --link kitconcept-blog-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d kitconcept/kitconcept-blog-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop kitconcept-blog-frontend-acceptance
	@docker stop kitconcept-blog-backend-acceptance
